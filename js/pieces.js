// Font Awesome Free 6.7.2 chess silhouettes, bundled locally under CC BY 4.0.
...
const svg = (type, { label, width, path }) => `
  <svg class="piece-svg piece-svg-${type.toLowerCase()}" viewBox="0 0 ${width} 512" role="img" aria-label="${label}" focusable="false">
    <g class="piece-shape"><path d="${path}" /></g>
  </svg>`;

export const PIECE_SVGS = Object.fromEntries(
  Object.entries(ICONS).map(([type, icon]) => [type, svg(type, icon)]),
);
...
export function decorativePiece(glyph, tone = "light") {
  const type = GLYPH_TYPES[glyph] || "P";
  return `<span class="vector-piece-icon ${tone}" aria-hidden="true">${PIECE_SVGS[type]}</span>`;
}
